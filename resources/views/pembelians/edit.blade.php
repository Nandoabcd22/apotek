                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" readonly value="{{ $pembelian->no_pembelian }}">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('tanggal_pembelian') is-invalid @enderror" 
                                           id="tanggal_pembelian" name="tanggal_pembelian" value="{{ $pembelian->tanggal_pembelian->format('Y-m-d') }}" required>
                                    ...
                                    <select class="form-select @error('id_supplier') is-invalid @enderror" 
                                        ...
                                        @foreach ($suppliers as $supplier)
                                            <option value="{{ $supplier->id_supplier }}" 
                                                {{ $pembelian->id_supplier == $supplier->id_supplier ? 'selected' : '' }}>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('diskon') is-invalid @enderror" 
                                           id="diskon" name="diskon" value="{{ $pembelian->diskon }}" min="0" max="100" step="0.01">
                                    @error('diskon')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
@section('scripts')
    <script>
        ...
        const existingItems = @json($pembelian->details);
        let itemCount = 0;
                ...
                <tr class="item-row" data-item="${itemCount}">
                    <td>
                        <select class="form-select obat-select" name="items[${itemCount}][id_obat]" required onchange="updateItemPrice(${itemCount})">
                            ...
                            ${obats.map(o => `<option value="${o.id_obat}" data-harga="${o.harga_beli}" ${obatId === o.id_obat ? 'selected' : ''}>${o.nama_obat}</option>`).join('')}
                        </select>
                    </td>
                    <td>
